...
app.get('/image/:folder/:name', (req, res) => {
  fs.readFile(
    './image' + '/' + req.params.folder + '/' + req.params.name,
    (err, data) => {
...
app.post('/image/user/:name', upload.single('file'), async (req, res) => {
  ...
  await sharp(req.file.path)
    ...
    .jpeg({ quality: 50 })
    .toFile(
      path.resolve(__dirname, './image/profilePic', req.params.name + '.jpg')
    );
